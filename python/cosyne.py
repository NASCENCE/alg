...
class Cosyne:
        ...
        while self.numEvals + self.population_size <= max_evals and self.best_fitness < target_fitness:
            ...
            for n in range(self.population_size):
                if rand() > crossover_percentage:
                    ...
                    new_population.append(self.population[chosen] + randn(self.dim) * self.sigmas[chosen] * (uniform(0,1,self.dim) < noise_chance))
                    new_sigmas.append(self.sigmas[chosen] * (normal(1,.1)**1))
                else:
    ...
    def save(self, filename):
        utils.save_pickle_gzip([self.numEvals, self.best_found, self.center, self.best_fitness, self.mutation, self.population, self.sigmas, self.crossover_proportion], filename)
    ...
    def load(self, filename):
        ...
        self.numEvals, self.best_found, self.center, self.best_fitness, self.mutation, self.population, self.sigmas, self.crossover_proportion = a
...
if __name__ == "__main__":
    ...
    optim = Cosyne(zeros(30), .01, rosen)
    ...
    try:
        ...
        pass
    print optim.start(verbose=True, max_evals=100000)
